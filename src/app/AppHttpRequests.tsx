...
export const AppHttpRequests = () => {
  ...
  useEffect(() => {
    todolistsApi.getTodolistApi()
      .then((res) => {
        const todolists = res.data
        setTodolists(todolists)
        todolists.forEach((tl) => {
          tasksApi.getTaskApi(tl.id)
            .then((res) => {
  ...
  const changeTaskStatusHandler = (e: React.ChangeEvent<HTMLInputElement>, task: Task) => {
    ...
    tasksApi.updateTaskStatus({taskId: task.id, todolistId: task.todoListId, model})
      .then(() => {
  ...
  const changeTaskTitleHandler = (taskId: string, title: string, todolistId: string) => {
    tasksApi.updateTaskTitle({ taskId, title, todolistId })
      .then(() => {
